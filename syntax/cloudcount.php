...
class syntax_plugin_data_cloudcount extends syntax_plugin_data_table {
     ...
     */
    function syntax_plugin_data_cloudcount(){
    ...
    function connectTo($mode) {
        $this->Lexer->addSpecialPattern('----+ *cloudcount(?: [ a-zA-Z0-9_]*)?-+\n.*?\n----+',$mode,'plugin_data_cloudcount');
    }
    ...
    function render($format, Doku_Renderer $renderer, $data) {
        ...
        foreach($tags as $tag){
        	...
        	$output_tag_label = hsc($tag['value']) .'<sub>('.$tag['cnt'].')</sub>';
